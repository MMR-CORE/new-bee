...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String(optionNameAPIAddr, ":11633", "HTTP API listen address")
	cmd.Flags().String(optionNameP2PAddr, ":11634", "P2P listen address")
	...
	cmd.Flags().StringSlice(optionNameBootnodes, []string{"/ip4/178.79.132.147/tcp/11634/p2p/16Uiu2HAmPE6TdBbU1MyYQeEWjCdZVHER2aPGAXWS1S5t9yJnCPXe"}, "initial nodes to connect to")
	cmd.Flags().Bool(optionNameDebugAPIEnable, false, "enable debug HTTP API")
	cmd.Flags().String(optionNameDebugAPIAddr, ":11635", "debug HTTP API listen address")
	...
	cmd.Flags().String(optionNameTracingEndpoint, "127.0.0.1:16831", "endpoint to send tracing data")
	...
	cmd.Flags().String(optionNamePaymentThreshold, "100000000", "threshold in FBZZ where you expect to get paid from your peers")
	cmd.Flags().String(optionNamePaymentTolerance, "100000000", "excess debt above payment threshold in FBZZ where you disconnect from your peer")
	cmd.Flags().String(optionNamePaymentEarly, "10000000", "amount in FBZZ below the peers payment threshold when we initiate settlement")
	...
	cmd.Flags().String(optionNameSwapEndpoint, "ws://localhost:18546", "swap ethereum blockchain endpoint")
	...
	cmd.Flags().String(optionNameSwapInitialDeposit, "10000000000000000000000", "initial deposit if deploying a new chequebook")
